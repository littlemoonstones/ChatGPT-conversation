...
import type { ChatMessage, Data, Conversation } from './lib/Type'
...
const switchConversation = (id: string) => {
  chatRoomStore.loadMessageObject(id)
}
...
const sendMessage = async (text: string) => {
  ...
  if (chatRoomStore.current_message_list.length == 0) {
...
const regenerateResonpse = async () => {
  ...
  (i.e., chatRoomStore.current_message_list.length - 2).
  */
  let lastUserNode =
    chatRoomStore.current_message_list[chatRoomStore.current_message_list.length - 2]
...
const clickDeleteConversation = (index: number, id: string) => {
  chatRoomStore.deleteConversation(id)
...
const clickClearAllConversation = () => {
  if (confirm('Are you sure delete all conversation?')) {
    listStore.List.forEach((item) => chatRoomStore.deleteConversation(item.id))
    ...
    <div class="overflow-hidden flex flex-1 flex-col h-full bg-black-100">
      ...
      <div class="flex flex-1 min-h-screen border">
        ...
        <div class="w-3/4 bg-base-100 flex flex-col justify-between">
          <div
            v-if="chatRoomStore.current_message_list.length == 0"
          ...
          <div v-else class="h-full bg-base-100 overflow-y-auto relative">
            <MessageComponent
              v-for="(message, index) in chatRoomStore.current_message_list"
              :index="index"
              :message="message"
              :key="message.id"
            />
